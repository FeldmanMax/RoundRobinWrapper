...
class RetryMechanism @Inject()() {
  ...
  private def executeImpl[T <: AnyRef](connectionName: String,
                                       ...
                                       action: () => Either[Exception, T]): Either[String, RetryMechanismResult[T]] = {
    if(retry == 0)  {
      Left(s"Could not bring result for $connectionName connection\nReasons\n" + (listOfErrors.map(_.getMessage) mkString "\n"))
    }
    else {
      action() match {
        ...
        case Left(exception) =>
          exception match {
            case custom: HttpCallCustomException => custom.errorType match {
              case NonRecoverable =>  Left(s"Had an unrecoverable problem ${custom.getMessage()}")
